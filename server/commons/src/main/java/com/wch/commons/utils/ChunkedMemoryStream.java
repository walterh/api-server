...
public class ChunkedMemoryStream implements Disposable {
    ...
    public ChunkedMemoryStream() throws Exception {
        this(0);
    }

    public ChunkedMemoryStream(int capacity) throws Exception {
        this(capacity, DefaultBlockAllocationSize);
    }

    public ChunkedMemoryStream(int capacity, int maxBlockSize) throws Exception {
        if (capacity < 0) {
    ...
    public static ChunkedMemoryStream safeCreate(byte[] bytes) {
        ChunkedMemoryStream bms = null;
        if (bytes != null) {
            try {
                bms = new ChunkedMemoryStream(bytes.length);
    ...
    public static void safeDispose(ChunkedMemoryStream bms) {
        if (bms != null) {
